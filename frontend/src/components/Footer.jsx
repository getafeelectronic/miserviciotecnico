...
function Footer() {
  ...
  // Datos de fallback para redes sociales
  ...
  // Datos de fallback para servicios destacados
  ...
  useEffect(() => {
    // Cargar redes sociales desde Supabase
    async function loadSocialLinks() {
      const data = await getSocialLinks();
      if (data && data.length > 0) {
        setSocialLinks(data);
      } else {
        console.info('📱 Usando redes sociales de fallback');
        setSocialLinks(fallbackSocialLinks);
      }
      setLoadingSocial(false);
    }

    // Cargar servicios destacados desde Supabase (solo 3 primeros)
    async function loadFeaturedServices() {
      const data = await getFeaturedServices();
      if (data && data.length > 0) {
        // Limitar a 3 servicios para el footer
        setFeaturedServices(data.slice(0, 3));
      } else {
        console.info('🔧 Usando servicios destacados de fallback');
        setFeaturedServices(fallbackFeaturedServices);
      }
        ...
        <div className="footer-section">
          ...
          <div className="social-links">
            {loadingSocial ? (
              ...
              // Renderizar redes sociales dinámicamente
